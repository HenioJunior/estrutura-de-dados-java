public class AlgoritmosDeOrdenacao {
	...
	public void insertionSort() {
		int carta, it, j;
		//a iteração vai selecionar a carta da vez e ai, tenho que "puxar" todos os elementos que são maiores
		for (it = 1; it < vetor.length; it ++) {
			carta = vetor[it];//guardo a carta
			for(j = it-1; (j >=0) && vetor[j] > carta; j--) {
				vetor[j+1] = vetor[j];//estou trocando todos que são maiores
			}
			vetor[j+1] = carta;
